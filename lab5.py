...
def db_connect():
    ...
    if db_type == 'postgres':
        conn = psycopg2.connect(
            ...
            database=os.getenv('POSTGRES_DB', 'gerakl_afanasov_knowledge_base'),
            user=os.getenv('POSTGRES_USER', 'postgres'),
            password=os.getenv('POSTGRES_PASSWORD', 'Gerakl2288'),
...
def edit_article(article_id):
    if 'username' not in session:
        ...
        return redirect(url_for('lab5.login'))
    conn, cur = db_connect()
    cur.execute("SELECT * FROM articles WHERE id = ? AND user_id = (SELECT id FROM users WHERE login = ?)", (article_id, session['username']))
...
def delete_article(article_id):
    if 'username' not in session:
        ...
        return redirect(url_for('lab5.login'))
    conn, cur = db_connect()
    cur.execute("DELETE FROM articles WHERE id = ? AND user_id = (SELECT id FROM users WHERE login = ?)", (article_id, session['username']))
    db_close(conn, cur)
    return redirect(url_for('lab5.list_articles'))
...
# #         return redirect(url_for('lab5.login'))
# #     conn, cur = get_db_connection()
# #     cur.execute("SELECT * FROM articles WHERE user_id = (SELECT id FROM users WHERE login = ?)", (session['username'],))
# #     articles = cur.fetchall()
# #     close_db_connection(conn, cur)
# #     if not articles:
# #         flash('У вас нет ни одной статьи.')
...
# #         return redirect(url_for('lab5.login'))
# #     conn, cur = get_db_connection()
# #     cur.execute("SELECT * FROM articles WHERE is_favorite = True AND user_id = (SELECT id FROM users WHERE login = ?)", (session['username'],))
# #     articles = cur.fetchall()
# #     close_db_connection(conn, cur)
# #     return render_template('lab5/favorite_articles.html', articles=articles)
