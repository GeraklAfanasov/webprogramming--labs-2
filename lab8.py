...
def article_list():
    ...
    if current_user.is_authenticated:
        ...
        public_articles = articles.query.filter(articles.is_public == True).all()
        return render_template(
            'lab8/articles.html',
            public_articles=public_articles,
            search_query=search_query,
...
#     password_form = request.form.get('password')
#     if not login_form:
#         return render_template('lab8/login.html', error='Пустой логин!')
#     if not password_form:
#         return render_template('lab8/login.html', error='Пустой пароль!')
